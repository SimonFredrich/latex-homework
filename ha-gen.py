...
due_to = input("Abgabetermin: ")

# check due_to date and correct if necessary with additional zeros
due_to_split_list = due_to.split(".")
due_to = ""
for i in range(0, len(due_to_split_list)-1):
    if (len(due_to_split_list[i]) < 2):
        due_to += "0" + due_to_split_list[i] + "."
    else:
        due_to += due_to_split_list[i] + "."
due_to+=due_to_split_list[2]
        
# compose directory name
directory = due_to + "-" + topic
